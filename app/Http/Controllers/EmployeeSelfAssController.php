...
class EmployeeSelfAssController extends Controller
{
    ...
    public function index()
{
    if(\Auth::user()->can('manage appraisal'))
    {
        ...
        if($user->type == 'Employee')
        {
            ...
            $appraisals = HrmEmpSelfAss::where('branch', $employee->branch_id)
                ->where('employee', $employee->id)
                ->with(['branches','employees'])
                ->get();
        ...
        {
            $competencyCount = Competencies::where('created_by', '=', $user->creatorId())->count();
            $appraisals = HrmEmpSelfAss::where('created_by', '=', \Auth::user()->creatorId())
                ->with(['branches','employees'])
                ->get();
        }

        return view('employee-self-assessment.index', compact('appraisals','competencyCount'));
    }
    else
    {
    ...
    public function create()
    {
        ...
        $employee = Employee::where('email', $userEmail)->first();
            ...
            $performance     = PerformanceType::where('created_by', '=', $employee->created_by)->get();
            
            
            $brances = Branch::where('created_by', '=', $employee->created_by)->get();
            return view('employee-self-assessment.create', compact( 'brances', 'performance'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    {
        if (\Auth::user()->can('create emp self ass')) {
            $userEmail = \Auth::user()->email;
            $employee = Employee::where('email', $userEmail)->first();
            $monthYear = date('Y-m', strtotime($request->appraisal_date));
            $existingAppraisal = HrmEmpSelfAss::where('employee', $employee->id)
                ->where('appraisal_date', $monthYear)
                ->first();
            
            // Log the existing appraisal for debugging
            if ($existingAppraisal) {
                return redirect()->route('employee-self-assessment.index')->with('error', __('Appraisal for the selected month already exists.'));
            }
    
            $selfassessment = new HrmEmpSelfAss();
            $selfassessment->branch = $employee->branch_id;
            $selfassessment->employee = $employee->id;
            $selfassessment->appraisal_date = $monthYear;
            $selfassessment->rating = json_encode($request->rating, true);
    
            $selfassessment->save();
    
            return redirect()->route('employee-self-assessment.index')->with('success', __('Self assessment created.'));
        }
    
        return redirect()->route('employee-self-assessment.index')->with('error', __('Unauthorized action.'));
    }
