    ...
    public function index(Request $request)
{
    if (\Auth::user()->can('manage attendance')) {
        ...
        $employee = Employee::select('id', 'name')->where('created_by', \Auth::user()->creatorId());
        ...
        if (!empty($request->branch) || !empty($request->department) || !empty($request->employee_id)) {
            $attendanceEmployee->whereIn('employee_id', $employeeIds);
        } else {
            $attendanceEmployee->whereIn('employee_id', $employee->pluck('id'));
        }
        ...
        } elseif ($request->type == 'range' && !empty($request->start_date) && !empty($request->end_date)) {
            $attendanceEmployee->whereBetween('date', [$request->start_date, $request->end_date]);
        } else {
            $month = date('m');
            $year = date('Y');
            $start_date = "$year-$month-01";
...
{
    $departments = Department::where('branch_id', $request->branch_id)->pluck('name', 'id');
