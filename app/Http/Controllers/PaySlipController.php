    ...
    public function bulkDelete(Request $request)
{
    ...
    if (!empty($ids)) {
        PaySlip::whereIn('id', $ids)->delete();
        return response()->json(['success' => true, 'message' => 'Selected payslips deleted successfully.']);
    }

    return response()->json(['success' => false, 'message' => 'No payslips selected for deletion.']);
}
    ...
    public function search_json(Request $request)
{
    ...
    else
    {
        ...
        foreach ($paylip_employee as $employee) {
            ...
            $deduction = $this->calculateTotalDeduction($employee->id, $formate_month_year);
            ...
            if (Auth::user()->type == 'Employee') {
                if (Auth::user()->id == $employee->user_id) {
                    ...
                    $tmp[] = \Auth::user()->priceFormat($deduction);
...
private function calculateTotalDeduction($employee_id, $month_year)
{
    $payslipDetail = Utility::employeePayslipDetail($employee_id, $month_year);
    $totalDeduction = $payslipDetail['totalDeduction'];
    ...
    public function bulkPay(Request $request)
{
    ...
    if(empty($ids)) {
        return response()->json(['success' => false, 'message' => __('No records selected.')]);
    }
    ...
    return response()->json(['success' => true, 'message' => __('Bulk payment processed successfully.')]);
}
