    ...
    public function index()
{
    if (\Auth::user()->can('manage appraisal'))
    {
        ...
        if ($user->type == 'Employee')
        {
            ...
            $appraisals = Appraisal::where('created_by', '=', $user->creatorId())
                ->where('branch', $employee->branch_id)
                ->where('employee', $employee->id)
                ->with(['branches', 'employees'])
                ->get()
                ->map(function ($appraisal) {
                    $selfAssessment = HrmEmpSelfAss::where('employee', $appraisal->employee)
                        ->where('appraisal_date', date('Y-m', strtotime($appraisal->appraisal_date)))
                        ->first();

                    $appraisal->self_assessment = $selfAssessment ? json_decode($selfAssessment->rating, true) : null;
                    return $appraisal;
                });
        }
        else
